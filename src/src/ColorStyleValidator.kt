...
internal val NAMED_COLORS: Map<String, Color?> = sequenceOf(
    "transparent" to null,
    "aliceblue" to "#f0f8ff",
    ...
    .map { pair ->
        pair.first to pair.second?.let { Color.decode(it) }
    }
    .toMap(HashMap())
...
internal data object ColorStyleValidator : StyleValidator {
    ...
    override val completionElements = NAMED_COLORS.map { (name, color) ->
        LookupElementBuilder.create(name)
            ...
            .run {
                if (color == null) this else withIcon(ColorIcon(16, color))
            }
    }
    ...
    override fun isAcceptableValueType(elementType: IElementType): Boolean {
        return elementType == D2ElementTypes.COLOR || elementType == D2ElementTypes.UNQUOTED_STRING || elementType == D2ElementTypes.STRING
    }
}